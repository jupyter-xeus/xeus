...
#ifndef XEUS_HASH_HPP
#define XEUS_HASH_HPP
...
namespace xeus
{
    ...
    namespace detail
    {
        ...
        inline uint32_t murmur2_x86_impl(const void* buffer, std::size_t length, uint32_t seed)
        {
            ...
            while(len >= 4)
            {
                uint32_t k = *reinterpret_cast<uint32_t*>(const_cast<unsigned char*>(data));
                k *= m;
                k ^= k >> 24;
                k *= m;
        ...
        {
            constexpr std::size_t m = (std::size_t(0xc6a4a793) << 32) +
                std::size_t(0x5bd1e995);
            constexpr int r = 47;
            const char* data = static_cast<const char*>(buffer);
